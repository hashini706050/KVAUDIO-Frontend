...
export default function BookingPage(){
    ...
    function calculateTotal(){
        ...
        axios.post(`${import.meta.env.VITE_BACKEND_URL}/orders/quote`,
            cartInfo,{
        headers: {
            ...
            console.error(err);
            toast.error("Failed to calculate total – are you logged in?");
        })
    }
    ...
    function handleBookingCreation(){
        ...
        const token = localStorage.getItem("token");
        axios.post(`${import.meta.env.VITE_BACKEND_URL}/orders`, cart, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        }).then((res)=>{
  ...
  return(
        <div className="w-full h-full flex flex-col items-center ">
            ...
            <div className="w-full flex justify-center mt-4">
                <button className="bg-accent bg-blue-600 text-white px-4 py-2 rounded-md" onClick={handleBookingCreation}>Create Booking</button>
            </div>
        </div>
    )
}